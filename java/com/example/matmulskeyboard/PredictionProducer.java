...
public class PredictionProducer {
    ...
    public void addMessages(Text result){
        ...
        for(int x=textBlocks.size()-1;x>=0;x--) {
            Text.TextBlock lines = textBlocks.get(x);
            Point[] blockCornerPoints = lines.getCornerPoints();
            int center = (blockCornerPoints[0].x+blockCornerPoints[1].x)/2;
            if(Math.abs(center-middle_pixel)<5){ //Ignores text at the centre. Usually time and date of conversation.
                continue;
            }
            String text = lines.getText();
            if(last_message !=null) {
                hasNewInfo = !messages.isEmpty();
            ...
            if(unnecessaryText(text))
                continue;
            messages.add(text);
            if(center<middle_pixel) { //Remote messages are aligned to the left of the screen
                remLocal.add(true);
            }
